...
function broadcastThrottled(message) {
    ...
    if (!_broadcastTimeout) {
        _broadcastTimeout = setTimeout(() => {
            _broadcastTimeout = null;
            const messages = _pendingBroadcasts;
            _pendingBroadcasts = [];
            // Batch send: combine build-output messages
            const buildOutputs = messages.filter(m => m.type === 'build-output');
            const otherMessages = messages.filter(m => m.type !== 'build-output');

            // Send other messages immediately
            otherMessages.forEach(m => broadcast(m));

            // Send build outputs as a batch if there are many
            if (buildOutputs.length > 0) {
                if (buildOutputs.length === 1) {
                    broadcast(buildOutputs[0]);
                } else {
                    // Batch multiple build outputs into a single message
                    broadcast({
                        type: 'build-output-batch',
                        data: {
                            lines: buildOutputs.map(m => m.data.line),
                            stage: buildOutputs[buildOutputs.length - 1].data.stage,
                            progress: buildOutputs[buildOutputs.length - 1].data.progress
                        }
                    });
                }
            }
...
let _saveBuildStateTimeout = null;
let _pendingSave = false;

function saveBuildState() {
    _pendingSave = true;
    // Debounce: wait 500ms after last call before actually saving
    if (_saveBuildStateTimeout) {
        clearTimeout(_saveBuildStateTimeout);
    }
    _saveBuildStateTimeout = setTimeout(() => {
        _pendingSave = false;
        try {
            fs.writeFileSync(BUILD_STATE_FILE, JSON.stringify(buildState, null, 2));
        } catch (e) {
            console.error('[BuildState] Failed to save:', e.message);
        }
    }, 500);
}

// Force immediate save when needed (for critical state changes)
function saveBuildStateImmediate() {
    if (_saveBuildStateTimeout) {
        clearTimeout(_saveBuildStateTimeout);
    }
    _pendingSave = false;
...
function startBuild() {
    ...
    ptyProcess.onData((data) => {
        ...
        for (let line of lines) {
            ...
            // Efficient log truncation: remove oldest entries instead of slicing
            if (buildState.log.length > 5000) {
                // Remove first 2500 entries (more efficient than slice which creates new array)
                buildState.log.splice(0, 2500);
            }

            // Debounced save (every 50 entries or on stage change)
            ...
            // Use throttled broadcast to reduce CPU/network overhead
            broadcastThrottled({
                type: 'build-output',
                data: {
                    line,
