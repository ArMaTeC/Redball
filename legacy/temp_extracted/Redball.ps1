...
# --- IPC Server for WPF UI Communication ---
$script:ipcRunspace = $null
$script:ipcPowerShell = $null

function Start-IpcServer {
    <#
    .SYNOPSIS
        Starts a named pipe server to receive commands from the WPF UI.
    .DESCRIPTION
        Creates a named pipe server that listens for JSON messages from the WPF UI.
        Handles commands like ToggleActive to synchronize state between UI and core.
    #>
    try {
        Write-VerboseLog -Message "Starting IPC server on pipe 'RedballPS'" -Source "IPC"
        
        $script:ipcRunspace = [runspacefactory]::CreateRunspace()
        $script:ipcRunspace.Open()
        
        # Share the script scope variables with the runspace
        $script:ipcRunspace.SessionStateProxy.SetVariable('script:state', $script:state)
        $script:ipcRunspace.SessionStateProxy.SetVariable('script:config', $script:config)
        
        $script:ipcPowerShell = [powershell]::Create()
        $script:ipcPowerShell.Runspace = $script:ipcRunspace
            ...
            while ($true) {
                try {
                    ...
                    $pipe = [System.IO.Pipes.NamedPipeServerStream]::new(
                        ...
                        1,  # Max 1 simultaneous connection
                    ...
                    # Wait for connection (blocking call in runspace)
                        ...
                        if ($json) {
                            try {
                                ...
                                try {
                                    ...
                                    $timestamp = Get-Date -Format 'yyyy-MM-dd HH:mm:ss'
                                    $logEntry = "[$timestamp] [INFO] [IPC] Received action: $action"
                                    $fileStream = [System.IO.File]::Open($logPath, [System.IO.FileMode]::OpenOrCreate, [System.IO.FileAccess]::Write, [System.IO.FileShare]::Read)
                                    $fileStream.Seek(0, [System.IO.SeekOrigin]::End) | Out-Null
                                    $writer2 = [System.IO.StreamWriter]::new($fileStream)
                                    $writer2.WriteLine($logEntry)
                                    $writer2.Dispose()
                                    $fileStream.Dispose()
                                }
                                catch {
                                ...
                                switch ($action) {
                                    'ToggleActive' {
                                        ...
                                        # Call Set-KeepAwakeState via reflection since we're in a runspace
                                        try {
                                            $flags = if ($newState) {
                                                [uint32]0x80000003  # ES_CONTINUOUS | ES_SYSTEM_REQUIRED | ES_DISPLAY_REQUIRED
                                            }
                                            else {
                                                [uint32]0x80000000   # ES_CONTINUOUS only
                                            }
                                            [Win32.Power]::SetThreadExecutionState($flags) | Out-Null
                                        }
                                        catch {
                                            $response.Success = $false
                                            $response.Error = "SetThreadExecutionState failed: $_"
                                        }
                                    }
                                    default {
                                        $response.Success = $false
                                ...
                                $response = @{ Success = $false; Error = "Parse error: $_" }
                    ...
                    # Pipe error - wait before retry
                    Start-Sleep -Milliseconds 100
                }
            }
        }
        
        $script:ipcPowerShell.AddScript($ipcScript).AddArgument($script:state).AddArgument($script:config) | Out-Null
        $script:ipcPowerShell.BeginInvoke() | Out-Null
        
        Write-RedballLog -Level 'INFO' -Message 'IPC server started on pipe RedballPS'
        Write-VerboseLog -Message "IPC server started successfully" -Source "IPC"
    }
    catch {
        Write-RedballLog -Level 'WARN' -Message "Failed to start IPC server: $_"
        Write-VerboseLog -Message "IPC server start failed: $_" -Source "IPC"
    }
}
...
function Stop-IpcServer {
    <#
    .SYNOPSIS
        Stops the IPC server runspace.
    #>
    try {
        Write-VerboseLog -Message "Stopping IPC server" -Source "IPC"
        if ($script:ipcPowerShell) {
            $script:ipcPowerShell.Stop()
            $script:ipcPowerShell.Dispose()
            $script:ipcPowerShell = $null
        }
        if ($script:ipcRunspace) {
            $script:ipcRunspace.Close()
            $script:ipcRunspace.Dispose()
            $script:ipcRunspace = $null
        }
        Write-VerboseLog -Message "IPC server stopped" -Source "IPC"
    }
    catch {
        Write-VerboseLog -Message "IPC server stop error: $_" -Source "IPC"
    }
}

# Start IPC server for WPF UI communication
Start-IpcServer
