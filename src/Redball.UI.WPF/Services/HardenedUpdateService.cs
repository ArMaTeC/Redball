...
public class HardenedUpdateService
{
    ...
    /// </summary>
    public async Task<UpdateInfo?> CheckForUpdateWithRetryAsync(bool bypassCache = false, IProgress<UpdateCheckProgress>? progress = null, CancellationToken cancellationToken = default)
    {
        ...
        for (int attempt = 0; attempt <= RetryDelays.Length; attempt++)
        {
            ...
            try
            {
                ...
                var result = await _innerService.CheckForUpdateAsync(bypassCache, progress, cancellationToken);
