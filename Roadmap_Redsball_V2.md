...
This document presents a comprehensive 6-phase static analysis of the **Redball** desktop application codebase, which includes:

- **Update Server** (Node.js/Express): OTA update delivery and delta patching
- **Core Application** (C# .NET 10): WPF desktop app with Windows Service, Linux/Flatpak support

**Note**: This is a desktop application (WPF/C#), not a web application. The analysis framework has been adapted for desktop application concerns.
...
| 1.4 | `src/Redball.Core/Sync/HttpSyncApi.cs:46`   | API key sent in header without HTTPS enforcement check                          | Add HTTPS requirement check before sending credentials                         | [HIGH]     |

### High-Priority Issues

| #   | File/Location                                  | Issue Description                                                             | Proposed Fix                                                                 | Severity   |
| --- | ---------------------------------------------- | ----------------------------------------------------------------------------- | ---------------------------------------------------------------------------- | ---------- |
| 1.5 | `update-server/lib/delta-patches.js:56-70`     | `createPatch` uses synchronous `readFileSync` in async function               | Refactor to use `fs.promises.readFile` for non-blocking I/O                  | [HIGH]     |
| 1.6 | `src/Redball.UI.WPF/Services/Logger.cs:7`      | Console.WriteLine used for logging - doesn't integrate with system event logs | Implement Windows Event Log integration for service-tier logging             | [MEDIUM]   |
...
| 1.8 | `src/Redball.Service/IpcServer.cs:56`          | Named pipe security allows all authenticated users - no role-based access     | Implement ACL with specific group permissions (Administrators, RedballUsers) | [HIGH]     |
...
| 3.15 | `src/Redball.UI.WPF/Views/Pages/CrashTelemetryPage.xaml` | No opt-in/opt-out flow for crash reporting               | Add explicit consent dialog on first launch with granular controls | [HIGH]   |
...
| #   | File/Location                                               | Issue Description                                                           | Proposed Fix                                                          | Severity |
| --- | ----------------------------------------------------------- | --------------------------------------------------------------------------- | --------------------------------------------------------------------- | -------- |
| 4.1 | `src/Redball.Core/Sync/CrossPlatformAnalyticsSync.cs:48-55` | Analytics events appended synchronously to file - blocks calling thread     | Use in-memory queue with background batch flush (500ms or 100 events) | [HIGH]   |
| 4.2 | `src/Redball.UI.WPF/Services/AnalyticsService.cs`           | No debouncing on UI event tracking - floods event queue                     | Implement 250ms debounce for high-frequency events (scroll, resize)   | [HIGH]   |
...
| 4.4 | `src/Redball.UI.WPF/Services/ConfigService.cs`              | Config saved on every property change - causes excessive I/O                | Implement dirty flag with 5-second debounced save                     | [MEDIUM] |
...
| 4.6 | `src/Redball.Service/IpcServer.cs:76`          | StreamReader created per connection but not explicitly disposed | Add `using` statement for reader/writer in `HandleClientAsync`  | [MEDIUM] |
| 4.7 | `src/Redball.UI.WPF/Services/PluginService.cs` | Plugin assemblies loaded into default AppDomain - can't unload  | Implement `AssemblyLoadContext` for plugin isolation and unload | [HIGH]   |

### API/Network Performance

| #    | File/Location                             | Issue Description                                         | Proposed Fix                                             | Severity |
| ---- | ----------------------------------------- | --------------------------------------------------------- | -------------------------------------------------------- | -------- |
| 4.9  | `src/Redball.Core/Sync/HttpSyncApi.cs:52` | No connection pooling configuration - new TCP per request | Add `SocketsHttpHandler` with `PooledConnectionLifetime` | [MEDIUM] |
...
| #   | File/Location                                 | Issue Description                                                  | Proposed Fix                                                                       | Severity |
| --- | --------------------------------------------- | ------------------------------------------------------------------ | ---------------------------------------------------------------------------------- | -------- |
| 5.1 | `src/Redball.UI.WPF/Services/Logger.cs:29-30` | Mixed naming: `Warn` vs `Warning` - confusing API                  | Deprecate one, standardize on `Warning` (aligns with Microsoft.Extensions.Logging) | [POLISH] |
...
| #   | File/Location                            | Issue Description                                 | Proposed Fix                                                   | Severity |
| --- | ---------------------------------------- | ------------------------------------------------- | -------------------------------------------------------------- | -------- |
| 5.4 | `src/Redball.UI.WPF/Services/`           | Hash computation logic duplicated across services | Extract to `Redball.Core.Cryptography.HashUtility`             | [MEDIUM] |
...
| 5.11 | `update-server/`                            | Delta patch format is not documented                       | Add `PATCH_FORMAT.md` documenting binary format for C# client | [MEDIUM] |

### Magic Numbers & Configuration

| #   | File/Location                                         | Issue Description                            | Proposed Fix                                                 | Severity |
| --- | ----------------------------------------------------- | -------------------------------------------- | ------------------------------------------------------------ | -------- |
| 5.7 | `src/Redball.Service/IpcServer.cs:84`                 | `4096` buffer size magic number              | Define as named constant `DefaultPipeBufferSize`             | [POLISH] |
| 5.8 | `src/Redball.Core/Sync/OutboxDispatcherService.cs:35` | `maxRetries = 10` - no backoff configuration | Add `RetryPolicy` configuration object with backoff strategy | [MEDIUM] |
...
| #   | File/Location                                             | Issue Description                                                            | Proposed Fix                                                        | Severity   |
| --- | --------------------------------------------------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------- | ---------- |
| 6.1 | `src/Redball.Core/Sync/CrossPlatformAnalyticsSync.cs:14`  | Analytics enabled by default without explicit opt-in                         | Add first-run consent dialog with granular feature toggles          | [CRITICAL] |
...
| 6.3 | `src/Redball.UI.WPF/Services/AnalyticsService.cs`         | No data retention limit for local analytics storage                          | Implement 90-day default retention with cleanup job                 | [MEDIUM]   |
...
| #   | File/Location                                         | Issue Description                                          | Proposed Fix                                                              | Severity |
| --- | ----------------------------------------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------- | -------- |
| 6.5 | `src/Redball.UI.WPF/Services/AnalyticsService.cs`     | High-frequency events (mouse moves) could flood sync queue | Add aggressive sampling: 1% for mouse, 10% for scroll, 100% for clicks    | [HIGH]   |
...
| 6.7 | `browser-extension/content.js:33`                     | Activity check every 60 seconds even when user is active   | Reset timer on activity events instead of polling                         | [POLISH] |
| 6.8 | `src/Redball.Core/Sync/HttpSyncApi.cs:50`             | Every event triggers individual HTTP POST - inefficient    | Implement batch endpoint with 100-event buffer or 30-second flush         | [MEDIUM] |

### Data Minimization Issues

| #    | File/Location                                             | Issue Description                                          | Proposed Fix                                                | Severity |
| ---- | --------------------------------------------------------- | ---------------------------------------------------------- | ----------------------------------------------------------- | -------- |
| 6.9  | `src/Redball.Core/Sync/CrossPlatformAnalyticsSync.cs:30`  | Properties dictionary accepts any object - risk of PII     | Implement whitelist of allowed property types and names     | [HIGH]   |
...
| 6.11 | `src/Redball.Core/Telemetry/CrashTelemetryService.cs:89`  | Full stack trace may contain local file paths              | Sanitize stack traces to remove user-specific paths         | [MEDIUM] |
...
| 6.13 | `src/Redball.Core/Sync/CrossPlatformAnalyticsSync.cs` | No way to export or delete user data (GDPR/CCPA)         | Implement data portability and deletion APIs                  | [HIGH]   |
...
| ---------------------------------------------------- | --------- | -------------------------------- |
| Remove invalid `background` permission from manifest | Security  | Updated `manifest.json`          |
| Implement API URL configuration for extension        | Extension | Config service with discovery    |
...
| Create `injected.js` or remove from manifest         | Extension | File created OR manifest updated |
...
| Task                                             | Owner | Deliverable                     |
| ------------------------------------------------ | ----- | ------------------------------- |
| Add loading states to extension popup            | UX    | Skeleton UI components          |
| Implement theme support for extension            | UX    | CSS variable theming            |
...
| Create error state UI for extension              | UX    | Error view with troubleshooting |
...
| Add ESLint config for extension      | Code Quality  | `.eslintrc.js`             |
...
- **MISSING**: `injected.js` (referenced in manifest)
- **MISSING**: `icons/` directory (referenced but not found)
...
*Analysis scope: All codebase components*
